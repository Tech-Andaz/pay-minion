...
class BaadMayAPI
{
    ...
    public function createCheckoutLink($order, $response_type = "redirect"){
        ...
        if($this->BaadMayClient->success_url == "" && (!isset($order['success_url']) || $order['success_url'] == "")){
            throw new AlfalahIPGException("Success URL is missing. It can either be set once for all transactions or provided against each order or both.");
        }
        if($this->BaadMayClient->failure_url == "" && (!isset($order['failure_url']) || $order['failure_url'] == "")){
            throw new AlfalahIPGException("Failure URL is missing. It can either be set once for all transactions or provided against each order or both.");
        }
        $customer = array(
        ...
        $shipping = array(
            ...
            "cost" => (isset($order['shipping']['cost']) && $order['shipping']['cost'] != "") ? $order['shipping']['cost'] : 0,
